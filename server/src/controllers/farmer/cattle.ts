...
import { uploadBase64ToCloudinary, deleteFromCloudinary } from '../../services/cloudinaryService';
...
export const processTelemetry = async (telemetry: any, endpoint: string, success: boolean) => {
    try {
        ...
        if (formattedTelemetry.muzzleCropUrl && formattedTelemetry.muzzleCropUrl.startsWith('data:image')) {
            formattedTelemetry.muzzleCropUrl = await uploadBase64ToCloudinary(formattedTelemetry.muzzleCropUrl, 'ama-gau-dhana-telemetry');
        }
        if (formattedTelemetry.faceCropUrl && formattedTelemetry.faceCropUrl.startsWith('data:image')) {
            formattedTelemetry.faceCropUrl = await uploadBase64ToCloudinary(formattedTelemetry.faceCropUrl, 'ama-gau-dhana-telemetry');
        }
        ...
        console.error(`Failed to save AiLog from ${endpoint} telemetry:`, err);
    }
};
...
export const registerCow = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const faceTelemetryOci = await uploadImageToOCI(faceProfileFile.buffer, faceProfileFile.originalname, faceProfileFile.mimetype, 'ama-gau-dhana-telemetry');
        const muzzleTelemetryOci = await uploadImageToOCI(muzzleFile.buffer, muzzleFile.originalname, muzzleFile.mimetype, 'ama-gau-dhana-telemetry');
        ...
        try {
            await publishDlJob({
                ...
                farmer_id: farmerId,
                cow_name: name,
                face_image_oci: faceTelemetryOci,
                muzzle_image_oci: muzzleTelemetryOci
            });
            ...
            deleteFromCloudinary(faceProfileOci).catch(e => console.error(e));
            deleteFromCloudinary(muzzleOci).catch(e => console.error(e));
            if (leftProfileOci) deleteFromCloudinary(leftProfileOci).catch(e => console.error(e));
            if (rightProfileOci) deleteFromCloudinary(rightProfileOci).catch(e => console.error(e));
            if (backViewOci) deleteFromCloudinary(backViewOci).catch(e => console.error(e));
            if (tailViewOci) deleteFromCloudinary(tailViewOci).catch(e => console.error(e));
            if (selfieOci) deleteFromCloudinary(selfieOci).catch(e => console.error(e));
...
export const handleDlApiWebhook = async (req: Request, res: Response) => {
    try {
        const { cow_id, farmer_id, status, matched_cow_id, superpoint_cache, error_message, telemetry } = req.body;
        ...
        if (status === 'DUPLICATE') {
            ...
            // Delete root images because this is a rejected cow
            if (cow.photos.faceProfile) deleteFromCloudinary(cow.photos.faceProfile).catch(e => console.error(e));
            if (cow.photos.muzzle) deleteFromCloudinary(cow.photos.muzzle).catch(e => console.error(e));
            if (cow.photos.leftProfile) deleteFromCloudinary(cow.photos.leftProfile).catch(e => console.error(e));
            if (cow.photos.rightProfile) deleteFromCloudinary(cow.photos.rightProfile).catch(e => console.error(e));
            if (cow.photos.backView) deleteFromCloudinary(cow.photos.backView).catch(e => console.error(e));
            if (cow.photos.tailView) deleteFromCloudinary(cow.photos.tailView).catch(e => console.error(e));
            if (cow.photos.selfie) deleteFromCloudinary(cow.photos.selfie).catch(e => console.error(e));
